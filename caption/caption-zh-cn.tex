...
\def\thispackage{\package{caption}宏包}
\def\Thispackage{\package{caption}宏包}

\newcommand*\DefaultNoteText{笔记}
\newcommand*\NEWfeature{\NEW{新特性}}
\newcommand*\NEWdescription{\NEW{新的描述}}
\makeatletter
...
\begin{abstract}
\Thispackage\ 可以在一些浮动环境中的个性化制作标题，像图、表以及许多其它宏包。
\par\smallskip
\textit{请注意:} 许多类文件已经有了内置的个性化标题选项和命令。
如果你有足够能力的话，你根本不需要使用\package{caption}宏包。
而且如果你只对使用\cs{captionof}这个命令感兴趣的话，加载\package{capt-of}这个足够小的宏包已经足够了。
\end{abstract}

\newcommand\exampletext{%
  白色的沙滩。海螺壳的粉红色平滑度。有着各种可能性的海洋。装满欧洲最好的礼物和香水的免税商店。在一个热带果岭上玩你最喜欢的高尔夫球比赛的许多锦标赛课程。}

% --------------------------------------------------------------------------- %

\section*{Introduction}

在标准的 \LaTeX\ 类中标题没有得到应有的注意。它仅仅被当成一般的段落来排版，这和其它内容没有显著的视觉差，像这样:

\example*{size=normalsize,margin=0pt}{\exampletext}

我们可以改变这种情况；例如，可以使用比正常字体小一些的字，加一些额外的边距，使用和题目一样的字体系列和形状等等。像这个标题：

\example*{size=small,margin=10pt,labelfont=bf,labelsep=endash}{\exampletext}

使用本宏包你可以很容易的做到这些，因为这里已经定义好了很多标题格式选项，而且你也可以自由的定义你自己的选项。

\bigskip

\INFO\begin{minipage}[t]{\textwidth}
\small
请注意\thispackage\ 只是控制标题的外观以及给人的感觉。它\emph{不}控制标题的位置摆放。（但是你可以通过使用其它的像\package{floatrow}
宏包\cite{floatrow}来做到这点。）
\end{minipage}
...
\end{quote}
插入到你文档的导言区，也就是在|\document|\-|class| 和 |\begin{doc|\-|u|\-|ment}|之间。这个选项控制文档标题的外观；比如，
\begin{quote}
...
知道|\caption|\-|setup|只影响当前环境是有好处的。因此如果你只想改变当前|figure| 或者 |table|的设置，只需要把|\caption|\-|setup|命令放进|figure| 或 |table|环境中|\caption|命令的前面。你可以像下面这样
\begin{quote}
  |\begin{figure}|\\
  |  |\ldots\\
  ...
  |\end{figure}|
\end{quote}
关闭了single-line-check选项，但是仅仅对这个 |figure|起作用，
因此，所有其它的标题没有受到影响。

\begin{Annotation}
更加详细的关于 |\caption|\-|setup|命令的描述参见 \Ref{captionsetup}.
\end{Annotation}
