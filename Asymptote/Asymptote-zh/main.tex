...
\prgname{test.eps}。\verb=-V=\index{-V@\verb=-V=} 选项打开一个
...
\index{大点}\index{bp@\inlinecode=bp=}
\index{点}\index{pt@\inlinecode=pt=}
\index{英寸}\index{inch@\inlinecode=inch=}
在 \prgname{Asymptote} 中，像 |(0,0)| 和 |(100,100)| 这样的坐标称为%
\emph{复数}\transnote{原文为 pair，即序对。译文一般通称为复数。}%
\index{pair@\inlinecode=pair=}，以 \prgname{PostScript}“大点”\transnote{这
...
函数说明）：
\lstinputlisting{bigdiagonal.asy}
\begin{center}
  \includegraphics{bigdiagonal.pdf}
\end{center}
...
标签默认将位于路径的中点。另一种标签位置（在 |point(path p, real t)| 意义下）
可以在构造标签时用一个实数值 |position| 设定。位置
...
\index{identity@\inlinecode=identity=}
\index{Jn@\inlinecode=Jn=}
\index{Yn@\inlinecode=Yn=}
...
第一类 |Jn(int n, real)| 以及第二类 |Yn(int n, real)|，还有 $\Gamma$ 函数
